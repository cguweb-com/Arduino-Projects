...
void setup() {
  //delay to prevent IDE reset firing setup functions on upload/connect
  delay(6000); 
  ...
  //init pwm controller
  if (pwm_active) {
    if (debug) Serial.print(F("Setting up PWM Controller... "));
    if (buzz_active) {
      for (int b = 3; b > 0; b--) {
        tone(BUZZ, 10);
        delay(150);
...
//DEVWORK
      ...
      case 'h':
        ...
        Serial.println(F("\t|\tu\tstep_left\t\t|"));
        Serial.println(F("\t|\tj\tstep_right\t\t|"));
        ...
        Serial.println(F("\t|\t<\tz_dir +5\t\t|"));
        Serial.println();
        Serial.println(F("\t|\tz\tcurrent settings\t\t|"));
