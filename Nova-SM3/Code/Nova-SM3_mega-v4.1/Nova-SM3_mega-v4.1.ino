...
const byte skip_splash = 1;       //skip 15s of loading graphics
...
float x_dir_steps[2] = {-20, 20};
float y_dir_steps[2] = {-16, 16};
...
//vars used to compensate for uncalibrated front-to-back center of gravity
...
void setup() {
  ...
  delay(3000); //delay to prevent IDE reset firing setup functions on upload/connect
  ...
  if (pwm_oe) {
    ...
    if (!move_demo && !move_funplay) {
      ...
      if (ps2_select == 2) {
        ...
        x_dir = mapfloat(ps2x.Analog(PSS_LX), 0, 255, x_dir_steps[0], x_dir_steps[1]);
        z_dir = mapfloat(ps2x.Analog(PSS_RY), 0, 255, z_dir_steps[1], z_dir_steps[0]);
        move_steps = map(ps2x.Analog(PSS_RX), 0, 255, -move_steps_max, move_steps_max);

        //set move_steps to min 40 to maintain march-in-place
        //DEVNOTE: make this switchable via PS2
        if (move_steps == 0 || (move_steps > 0 && move_steps < 40)) {
          move_steps = 40;
        } else if (move_steps < 0 && move_steps > -40) {
          move_steps = -40;
        }
      }
...
//zdir = 0; // -50, +50
  ...
  //calculate move_steps and speeds, including wf (weight_factor compensation) for rear legs
//DEVNOTE: investigate using wf depending on walking direction, as I suspect the need for it on 
//         rear legs is not because of a dead-weight COG, but a moving-weight COG issue.
  float cms = (xdir * cmfact);
  float cms_wf = (xdir * (cmfact + (step_weight_factor * cmfact)));
  float csp = limit_speed((csfact * spd_factor));
  float csp_wf = limit_speed((csfact - (step_weight_factor * csfact)) * spd_factor);

  float fms = (move_steps * fmfact);
  float fms_wf = (move_steps * (fmfact + (step_weight_factor * fmfact)));
  float fsp = limit_speed((fsfact * spd_factor));
  float fsp_wf = limit_speed((fsfact - (step_weight_factor * fsfact)) * spd_factor);

  float tms = ((move_steps + ydir) * tmfact);
  float tms_wf = ((move_steps + ydir) * (tmfact + (step_weight_factor * tmfact)));
  float tsp = limit_speed((tsfact * spd_factor));
  float tsp_wf = limit_speed((tsfact - (step_weight_factor * tsfact)) * spd_factor);


  //to start sequencer, step first paired legs upwards (SEQ1)
  if (!activeServo[RFC] && !activeServo[RFF] && !activeServo[RFT] && !servoSequence[RF]) {
    //SEQ1 RF
    update_sequencer(RF, RFT, tsp, (gaitHome[RFT] - tms), (servoSequence[RF] + 1), 0);
    update_sequencer(RF, RFF, fsp, (gaitHome[RFF] + fms), servoSequence[RF], 0);
    update_sequencer(RF, RFC, csp, (gaitHome[RFC] + cms), servoSequence[RF], 0);

    //SEQ1 LR
    update_sequencer(LR, LRT, tsp_wf, (gaitHome[LRT] + tms_wf), (servoSequence[LR] + 1), 0);
    update_sequencer(LR, LRF, fsp_wf, (gaitHome[LRF] - fms_wf), servoSequence[LR], 0);
    update_sequencer(LR, LRC, csp_wf, (gaitHome[LRC] + cms_wf), servoSequence[LR], 0);
  }

  //when first paired complete, step second paired legs upwards (SEQ1), and first paired home (SEQ2)
  if (!activeServo[RFC] && !activeServo[RFF] && !activeServo[RFT] && servoSequence[RF] == 1) {
    //SEQ1 RR
    update_sequencer(RR, RRT, tsp_wf, (gaitHome[RRT] - tms_wf), (servoSequence[RR] + 1), 0);
    update_sequencer(RR, RRF, fsp_wf, (gaitHome[RRF] + fms_wf), servoSequence[RR], 0);
    update_sequencer(RR, RRC, csp_wf, (gaitHome[RRC] + cms_wf), servoSequence[RR], 0);

    //SEQ1 LF
    update_sequencer(LF, LFT, tsp, (gaitHome[LFT] + tms), (servoSequence[LF] + 1), 0);
    update_sequencer(LF, LFF, fsp, (gaitHome[LFF] - fms), servoSequence[LF], 0);
    update_sequencer(LF, LFC, csp, (gaitHome[LFC] + cms), servoSequence[LF], 0);

    //SEQ2 RF
    update_sequencer(RF, RFT, tsp, gaitHome[RFT], (servoSequence[RF] + 1), 0);
    update_sequencer(RF, RFF, fsp, gaitHome[RFF], servoSequence[RF], 0);
    update_sequencer(RF, RFC, csp, gaitHome[RFC], servoSequence[RF], 0);

    //SEQ2 LR
    update_sequencer(LR, LRT, tsp_wf, gaitHome[LRT], (servoSequence[LR] + 1), 0);
    update_sequencer(LR, LRF, fsp_wf, gaitHome[LRF], servoSequence[LR], 0);
    update_sequencer(LR, LRC, csp, gaitHome[LRC], servoSequence[LR], 0);
  }

  //when second paired complete, step second paired legs home (SEQ2), and reset sequencers
  if (!activeServo[LFC] && !activeServo[LFF] && !activeServo[LFT] && servoSequence[LF] == 1) {
    //SEQ 2 RR
    update_sequencer(RR, RRT, tsp_wf, gaitHome[RRT], (servoSequence[RR] + 1), 0);
    update_sequencer(RR, RRF, fsp_wf, gaitHome[RRF], servoSequence[RR], 0);
    update_sequencer(RR, RRC, csp, gaitHome[RRC], servoSequence[RR], 0);

    //SEQ 2 LF
    update_sequencer(LF, LFT, tsp, gaitHome[LFT], (servoSequence[LF] + 1), 0);
    update_sequencer(LF, LFF, fsp, gaitHome[LFF], servoSequence[LF], 0);
    update_sequencer(LF, LFC, csp, gaitHome[LFC], servoSequence[LF], 0);
  ...
  if (serial_active && Serial.available() > 0) {
    ...
    switch (ByteReceived) {      
      ...
      case 59:
        ...
        if (!plotter) { Serial.print(F(" y_dir: "));Serial.println(y_dir); }
        break;
      case 39:
        if (!plotter) Serial.print(F("y_dir +5\t"));
      ...
      case 46:
        ...
        if (!plotter) { Serial.print(F(" x_dir: "));Serial.println(x_dir); }
        break;
      case 47:
        if (!plotter) Serial.print(F("x_dir +5\t"));
      ...
      case 44:
        ...
        if (!plotter) { Serial.print(F(" z_dir: "));Serial.println(z_dir); }
        break;
      case 60:
        if (!plotter) Serial.print(F("z_dir +5\t"));
      ...
      case 'h':
        ...
        Serial.println();
        Serial.println(F("\t|\t[\tmove_steps -5\t\t|"));
        Serial.println(F("\t|\t]\tmove_steps +5\t\t|"));
        Serial.println(F("\t|\t;\ty_dir -5\t\t|"));
        Serial.println(F("\t|\t'\ty_dir +5\t\t|"));
        Serial.println(F("\t|\t.\tx_dir -5\t\t|"));
        Serial.println(F("\t|\t/\tx_dir +5\t\t|"));
        Serial.println(F("\t|\t,\tz_dir -5\t\t|"));
        Serial.println(F("\t|\t<\tz_dir +5\t\t|"));
        Serial.println();
